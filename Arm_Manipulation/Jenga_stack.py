...
def odd_level(x,y,z, block, level, height):
    if block==1:
        ...
        level=level+1
        height=height+0.03
    return block, level, height

def even_level(x,y,z, block, level, height):
    if block==1:
        bot.arm.set_ee_pose_components(x=x, y=y, z=z+0.05, pitch=1.5)
        # bot.arm.set_ee_pose_components(x=x, y=y, z=z, pitch=0.5)
        bot.gripper.close()
        ...
        # bot.arm.set_ee_pose_components(x=x, y=y, z=z+0.05, pitch=1.5)
        bot.arm.set_ee_pose_components(x=0.38, y=-0.22, z=0.04+height,roll=-1.45, pitch=1.5)#target coordinates
        bot.gripper.open()
...
def main():
    ...
    for cluster in clusters:
        ...
        if level%2!=0:
            block, level, height =  odd_level(x,y,z, block, level, height)
        else:
            block, level, height = even_level(x,y,z, block, level, height)  
